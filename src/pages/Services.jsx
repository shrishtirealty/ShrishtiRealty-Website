...
export default function Services() {
  return (
    <>
      <PageBanner label="Our Expertise" title="Our Services" subtitle="Why Partner With Us"
        img="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=1920&q=80" />

      {/* Intro */}
      <section className="py-24 lg:py-32 bg-white">
        <div className="max-w-3xl mx-auto px-5 text-center mb-20">
          <Reveal>
            <h2 className="font-display text-3xl lg:text-4xl font-bold text-gray-900 mb-5">
              A Full-Spectrum Design-Build Brand
            </h2>
            <p className="text-gray-600 leading-[1.9]">
              From concept to completion, we deliver excellence across seven core verticals —
              each powered by decades of expertise, global perspective, and an unwavering
          ...
          <div className="space-y-6">
            {services.map((s, i) => (
              <Reveal key={i} delay={i * 0.05}>
                <Link to={s.to} className={`group grid md:grid-cols-[1fr_1.5fr] gap-0 rounded-2xl overflow-hidden border border-gray-100 bg-white hover:shadow-xl hover:shadow-black/5 transition-all duration-500 ${i % 2 === 1 ? 'md:[direction:rtl]' : ''}`}>
                  {/* Image */}
                  <div className="relative h-[220px] md:h-auto md:min-h-[260px] overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-dark-green/20 group-hover:bg-dark-green/10 transition-all" />
                    {/* Number badge */}
                    <div className="absolute top-4 left-4 w-10 h-10 rounded-full bg-white/90 backdrop-blur grid place-items-center">
                      <span className="font-display text-sm font-bold text-dark-green">0{i + 1}</span>
                    </div>
                  </div>
                  {/* Content */}
                  <div className={`p-8 lg:p-10 flex flex-col justify-center ${i % 2 === 1 ? 'md:[direction:ltr]' : ''}`}>
                    <div className="w-10 h-10 rounded-xl bg-green-pale grid place-items-center mb-5 group-hover:bg-gold/10 transition-colors">
                      <s.icon className="text-green text-lg group-hover:text-gold transition-colors" />
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-dark-green text-[0.72rem] font-bold tracking-[0.1em] uppercase group-hover:text-gold group-hover:gap-3 transition-all duration-300">
                      Explore Service <FiArrowRight size={13} />
                    </div>
                  </div>
                </Link>
              </Reveal>
