    ...
    >
      <div className="max-w-[1400px] mx-auto px-5 lg:px-10 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-2.5">
          <a href="tel:+917498388944" className={`w-9 h-9 rounded-full border grid place-items-center transition-all duration-300 hover:scale-110 ${scrolled ? 'border-gray-200 text-gray-400 hover:border-gold hover:text-gold' : 'border-white/20 text-white/60 hover:border-gold hover:text-gold'}`} aria-label="Call"><FiPhone size={13} /></a>
          <a href="https://wa.me/7498388944" target="_blank" rel="noopener noreferrer" className={`w-9 h-9 rounded-full border grid place-items-center transition-all duration-300 hover:scale-110 ${scrolled ? 'border-gray-200 text-gray-400 hover:border-green hover:text-green' : 'border-white/20 text-white/60 hover:border-green-light hover:text-green-light'}`} aria-label="WhatsApp"><FaWhatsapp size={14} /></a>
          <button onClick={onConsultationClick}
            className={`ml-2 group relative overflow-hidden px-6 py-2.5 text-[0.68rem] font-bold tracking-[0.15em] uppercase rounded transition-all duration-500 ${
              scrolled ? 'bg-dark-green text-white hover:shadow-lg' : 'bg-gold text-dark-green hover:shadow-[0_4px_20px_rgba(201,168,76,0.3)]'
            }`}>
            <span className="relative z-10">Book A Consultation</span>
            <span className={`absolute inset-0 transition-transform duration-500 origin-left scale-x-0 group-hover:scale-x-100 ${scrolled ? 'bg-green' : 'bg-gold-light'}`} />
          </button>
        </div>

        <button className={`lg:hidden p-2 z-[101] ${scrolled ? 'text-gray-800' : 'text-white'}`} onClick={() => setMob(!mob)} aria-label="Menu">
            ...
            <motion.div className="fixed top-0 right-0 w-[85%] max-w-[380px] h-screen bg-white z-[99] shadow-2xl" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 220 }}>
              <div className="pt-20 px-6 pb-8 h-full overflow-y-auto">
                ...
                <div className="mt-8 pt-6 border-t border-gray-100">
                  <button onClick={onConsultationClick} className="w-full py-3 bg-dark-green text-white text-[0.75rem] font-bold tracking-[0.15em] uppercase rounded hover:bg-green transition-colors">Book A Consultation</button>
                </div>
              </div>
            </motion.div>
          </>
...
function DD({ label, scrolled, isHome, active, open, onE, onL, children }) {
  ...
  return (
    <div className="relative" onMouseEnter={onE} onMouseLeave={onL}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
            className="absolute top-full left-0 mt-3 w-72 bg-white rounded-2xl py-3 shadow-[0_25px_60px_rgba(0,0,0,0.12)] border border-black/[0.04]">
            {children}
          </motion.div>
        )}
      </AnimatePresence>
