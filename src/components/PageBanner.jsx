      ...
      <div className="relative z-10 text-center px-5 py-28">
        {label && (
          <motion.div className="flex items-center justify-center gap-3 mb-5" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <span className="text-[0.58rem] font-semibold tracking-[0.4em] uppercase text-gold/80">{label}</span>
            <div className="h-px w-8 bg-gold/50" />
          </motion.div>
        )}
        <div className="overflow-hidden">
          <motion.h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-normal text-white tracking-[-0.02em]"
            initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ delay: 0.35, duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>
            {title}
          </motion.h1>
        </div>
        {subtitle && (
          <motion.p className="text-sm md:text-base text-white/55 mt-4 max-w-lg mx-auto font-light tracking-wide"
            initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
            {subtitle}
          </motion.p>
        )}
        <motion.div className="w-14 h-[1.5px] bg-gold/50 mx-auto mt-6" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.8, duration: 0.6 }} />
      </div>
    </section>
  )
}
